<template>
	<section class="role-management">
		<div class="bg-light lter b-b wrapper-md"><h1 class="m-n font-thin h3">菜单管理</h1></div>
		<div class="wrapper-md">
			<!-- search bar -->
		  <el-row :gutter="20">
		  	<el-col :span="19">
				  <el-autocomplete class="inline-input" v-model="searchValue" :fetch-suggestions="querySearch" placeholder="请输入需要查找的菜单" :trigger-on-focus="false" @select="handleSelect" style="width: 100%;">
				  	<!--<el-button slot="append" icon="search"></el-button>-->
				  </el-autocomplete>
		  	</el-col>
 				<el-col :span="5">
 					<el-button type="primary" icon="plus" @click="dialogFormVisible = true">新增菜单</el-button>
 				</el-col>
		  </el-row>
		  <!-- / search bar -->
		  ...
		  <el-row>
		  	<el-table :data="currentData" v-loading.body="loading" style="width: 100%" :row-class-name="filterRowDisable" border>
			    <el-table-column prop="name" label="菜单名" :formatter="filterEmpty"></el-table-column>
			    <el-table-column prop="level" label="级别" :formatter="filterEmpty"></el-table-column>
			    <el-table-column prop="parentid" label="父菜单" :formatter="filterEmpty"></el-table-column>
			    <el-table-column prop="path" label="路径" :formatter="filterEmpty"></el-table-column>
		...
		<!-- add component -->
		<el-dialog title="新增菜单" v-model="dialogFormVisible" @close="handleReset">
			<el-form :model="form" :rules="formrules" ref="form" label-width="80px">
				<el-form-item label="菜单名" prop="name">
					<el-input v-model="form.name" auto-complete="off" placeholder="例：详情页"></el-input>
				</el-form-item>
				<el-form-item label="描述" prop="description">
					<el-input v-model="form.description" auto-complete="off" placeholder="例：新闻内容详情包含图文"></el-input>
				</el-form-item>
				...
				<el-form-item label="父菜单" prop="parentid" v-if="form.level > 0">
					<el-select v-model="form.parentid" placeholder="请选择父菜单">
						<el-option v-for="item in sortMainData.filter(e => e.level === 0)" :label="item.name" :value="item.menuid"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="路径" prop="path">
					<el-input v-model.number="form.path" auto-complete="off" placeholder="例：/somepath"></el-input>
				</el-form-item>
			</el-form>
...
<script>
	...
	export default {
        ...
        excludeEdit: ['menuid', 'updatetime', 'createtime', 'operator', 'parentid', 'level'], 
        formrules: {
          name: [
            { required: true, 
            	message: '请输入菜单名', 
      ...
      _filterPath (e) {
      	if (e.parentid > 0) {
      		e.path = this.mainData.find(f => f.menuid === e.parentid).path + e.path
      	}
      	return e
      },
      handleSubmit () {
      	handleSubmit.call(this, {
      		...
      		data: {
      			_token: this.token,
      			menuArray: this._filterPath(this.form)
      		}
      	})
      },
