<template>
    <section class="customer-management">
        <div class="bg-light lter b-b wrapper-md"><h1 class="m-n font-thin h3">快速入口管理</h1></div>
                ...
                <el-card class="box-card" v-for="(item, index) in form.items">
                    ...
                    <div>
                        ...
                        <el-form-item label="更多">
                            <el-input v-model="item.more" auto-complete="off" placeholder="例：更多的连接"></el-input>
                        </el-form-item>
                        <el-form-item label="图标" :prop="'items.' + index + '.icon'" :rules="{required: true, trigger: 'change'}">
                            <el-input v-model="item.icon" auto-complete="off"></el-input>
                            <el-upload action="/api/upload/expressway" :on-success="handleSuccess.bind(undefined, index, 'icon')" :on-error="handleuploaderror" accept="image/*" :before-upload="beforeUpload" list-type="picture" :show-file-list="false">
                                <div v-if="item.icon" class="picProview">
                                    <img :src="item.icon">
                                </div>
